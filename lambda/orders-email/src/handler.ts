...
async function releaseEmailSendLock(keys: { businessId: string; createdAtOrderId: string }): Promise<void> {
  try {
    await ddb.send(
      new UpdateCommand({
        TableName: ORDERS_TABLE,
        Key: keys,
        UpdateExpression: 'REMOVE emailSendLockId, emailSendLockAt',
        ConditionExpression: 'attribute_exists(emailSendLockId) AND attribute_not_exists(emailSentAt)',
      })
    );
  } catch (err) {
    if (conditionalCheckFailed(err)) return;
    throw err;
  }
}
...
async function processRecord(record: DynamoDbStreamRecord, resend: Resend): Promise<void> {
  ...
  if (!customerEmail || !businessEmail) {
    ...
    await releaseEmailSendLock(keys);
    return;
  }
...
export async function handler(event: DynamoDbStreamEvent): Promise<void> {
  ...
  if (failures) {
    ...
    throw new Error(`[orders-email] batch failed (${failures} records)`);
  }
}
